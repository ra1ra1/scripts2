...
class Mouse():
    ...
    def get_offset(self):
	if self.cnt >= self.wait_cnt:
	    if self.cnt % 50 == 0:
	        self.update_offset()

	if self.cnt >= 1000000: self.cnt = 0
	self.cnt += 1
	...
	if self.vel.linear.x == 0.0 and self.vel.angular.z == 0.0 : self.v_imu = 0.0
    ...
    def plot(self):
	self.t += CTR_P
	write_data(self.t, self.ax, self.offset, self.v_imu, self.v_odom, 'a')
...
def init_data():
    write_data(0,0,0,0,0,'w')

def write_data(t,data1,data2,data3,data4,op):
    f=open(DATA ,op)
    f.write(str(t) + "," + str(data1) + "," + str(data2) + "," + str(data3) + "," + str (data4) + "\n")
    f.close()
