...
class Mouse():
	...
	self.wait_cnt = 50
	self.flag = 0
	self.offset, self.offset2 = 0.0, 0.0
	self.offset_flag = 0
	self.imu_data = Imu()
	self.vel = Twist()
    ...
    def get_offset(self):
	if self.cnt % self.wait_cnt == 0:
	    self.wait_cnt = 50
	    self.v_imu = 0.0
	    if self.n <= 10000 :self.update_offset()
	if self.cnt >= 10000:
	    self.cnt = 1
	self.cnt += 1
	...
	self.offset2 = self.offset2 + Kn * (self.imu_data.angular_velocity.z - self.offset2)
        self.n += 1
        self.pub5.publish(self.offset)
	#print(self.offset, self.offset2)
	print(self.imu_data.angular_velocity.z, self.omega)
    ...
    def calc_v(self):
	#self.v_imu += self.ax * T_CTR
	if self.vel.linear.x == 0.0:
	    self.v_imu = 0.0
	    self.filtered_v_imu = 0.0
	else: 
	    self.v_imu += self.ax * T_CTR
	    self.filtered_v_imu += self.filtered_ax * T_CTR
	...
	write_data(self.t, self.ax ,self.v_imu, self.filtered_ax ,self.filtered_v_imu, self.vel.linear.x, 'a')
	self.t += T_CTR
	self.cnt += 1
	#self.last_time = self.cur_time
    ...
    def get_omega(self):
	self.omega = self.imu_data.angular_velocity.z - self.offset2
        #self.filter_omega()
...
def write_data(t,data1,data2,data3,data4,data5,op):
    f=open(DATA ,op)
    f.write(str(t) + "," + str(data1) + "," + str(data2) + "," + str(data3) + "," + str(data4) + "," + str(data5) + "\n")
    f.close()
